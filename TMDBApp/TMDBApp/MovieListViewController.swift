...
class MovieListViewController: UIViewController {
    ...
    var movieList: [Result] = [] // 파싱한 데이터를 저장할 배열
        ...
        AF.request("https://api.themoviedb.org/3/movie/now_playing?api_key=\(PrivateKey.APIKey)", headers: headers).responseJSON { response in
            ...
            case .success(let value):
                do {
                    ...
                    let movieListModel = try decoder.decode(MovieListModel.self, from: jsonData)
                    self.movieList = movieListModel.results
                    print("self.movieList: \(self.movieList)")
                    self.tableView.reloadData() // 테이블 뷰 데이터 업데이트
                } catch {
                    print(error)
                }
...
extension MovieListViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        print("Selected: \(movieList[indexPath.row])")
    }
